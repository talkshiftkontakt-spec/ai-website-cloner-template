...
export function CrossedOutOverlay({ columns = 5, rows = 4 }: CrossedOutOverlayProps) {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-10 overflow-hidden" aria-hidden>
      ...
      <svg
        viewBox="0 0 100 100"
        className="absolute inset-0 h-full w-full text-primary opacity-90"
      ...
      >
        {marks.map((mark) => (
          <div key={mark} className="flex items-center justify-center">
            <span className="font-[family-name:var(--font-display)] text-2xl font-black leading-none text-primary/80 sm:text-3xl md:text-4xl lg:text-5xl">
              X
            </span>
          </div>
        ))}
