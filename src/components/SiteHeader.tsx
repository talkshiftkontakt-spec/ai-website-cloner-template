...
export function SiteHeader() {
  ...
  return (
    ...
    >
      <input type="checkbox" id={MOBILE_NAV_ID} className="sr-only" />

      <div className="gp-header-bar mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-6 lg:px-8">
        ...
        <label
          ...
          className="relative z-[70] flex h-12 w-12 shrink-0 cursor-pointer items-center justify-center rounded text-white touch-manipulation [-webkit-tap-highlight-color:transparent] lg:hidden"
          aria-label="Menu"
        >
          <span className="gp-menu-icon-open flex items-center justify-center">
            <svg className="pointer-events-none h-6 w-6" viewBox="0 0 1000 1000" fill="currentColor" aria-hidden>
          ...
          <span className="gp-menu-icon-close hidden items-center justify-center">
            <svg className="pointer-events-none h-6 w-6" viewBox="0 0 1000 1000" fill="currentColor" aria-hidden>
      ...
      <label
        htmlFor={MOBILE_NAV_ID}
        className="gp-mobile-backdrop fixed inset-0 top-16 z-[55] bg-black/40 lg:hidden"
        aria-hidden
      />

      <nav
        className="gp-mobile-nav fixed inset-x-0 top-16 z-[60] max-h-[calc(100dvh-4rem)] overflow-y-auto border-t border-white/10 bg-[#1f2b28] px-6 py-4 shadow-2xl lg:hidden"
...
function closeMobileMenu() {
  const toggle = document.getElementById(MOBILE_NAV_ID);
